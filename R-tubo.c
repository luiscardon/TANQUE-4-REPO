...
FILE * logg;
logg = fopen("R-tubo.log", "w");

fprintf(logg,"===========R-tubo.c ====================\n");
fprintf(logg,"LUS CARDON 2020 \n");
fprintf(logg," Prueba de funcion Gnielisnsky y estructuras .\n");

fprintf(logg,"Programa R-tubo.c \n");
fprintf(logg,"En i7/TANQUEs-4/TANQUE-4-REPO. Compilado con LIBSE. \n");
fprintf(logg,"Archivo: R-tubo.log\n");
...
Rp=log( (ri+e)/ri) / (2.*M_PI*kp);
printf("Rp   %f\n",Rp);

Rs=log( (2.*re)/(ri+e) )/(2*M_PI*ks);
printf("Rs   %f\n",Rs);
R=Rc +Rs + Rp;
printf("R   %f\n",R);
